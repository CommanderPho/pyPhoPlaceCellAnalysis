...
# and `pip install pyPhoPlaceCellAnalysis` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "pyPhoPlaceCellAnalysis", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n pyPhoPlaceCellAnalysis -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "pyPhoPlaceCellAnalysis", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "pyPhoPlaceCellAnalysis", "YOUR_CONSOLE_SCRIPT"]
