...
@function_attributes(short_name=None, tags=['peak_prominence'], input_requires=[], output_provides=[], uses=[], used_by=['_render_posterior_peak_prominence_2d_results_on_pyvista_plotter'], creation_date='2026-01-07 07:00', related_items=[])
def _build_pyvista_single_posterior_prominence_result_data(epoch_idx, time_bin_idx, a_result, promenence_plot_threshold = 1.0, included_level_indicies=[1], debug_print=False):
    """
    Builds peak prominence result data for a single posterior (epoch_idx, time_bin_idx) tuple.
    
    Similar to _build_pyvista_single_neuron_prominence_result_data but works with PosteriorPeaksPeakProminence2dResult
    which is keyed by (epoch_idx, time_bin_idx) tuples instead of neuron_id integers.
    """
    ...
    n_peaks = len(peaks)
    if debug_print:
        print(f'posterior (epoch_idx={epoch_idx}, time_bin_idx={time_bin_idx}) - : {n_peaks} peaks:')
        ...
        if is_included:
            ...
            for level_idx, level_value in enumerate(filtered_levels_list):
                ...
                if curr_contour is not None:
                    ...
                    print(f"contour missing for posterior (epoch_idx={epoch_idx}, time_bin_idx={time_bin_idx}) - peak_id: {peak_id} - slice[{level_value}]. Skipping.")
        else:
            print(f'\tskipping posterior (epoch_idx={epoch_idx}, time_bin_idx={time_bin_idx}) - peak_id: {peak_id} because prominence: {prominence} is too low.')
    return peak_locations[is_included_array,:], prominence_array[is_included_array], peak_labels, peak_levels, peak_level_bboxes
...
def _plot_pyvista_single_posterior_prominence_result(plotter, posterior_pdata, peak_locations, peak_labels, peak_levels, peak_level_bboxes, z_scaling_factor: float, active_curve_color, identifier_suffix: Optional[str]=None, render:bool=True, debug_print:bool=False, include_text_labels:bool=False, include_contour_bounding_box:bool=False, **kwargs):
    """ Plots a single set of contours/points/labels for a posterior
    
    Similar to _plot_pyvista_single_neuron_prominence_result but works directly with a plotter and posterior mesh
    instead of requiring an ipcDataExplorer.
    
    Args:
        plotter: PyVista plotter instance
        posterior_pdata: PyVista mesh for the posterior (from data_dict['grid'])
        peak_locations: Array of peak locations (n_peaks, 3)
        peak_labels: List of peak labels
        peak_levels: Array of peak level values (n_peaks, n_levels)
        peak_level_bboxes: Array of bounding boxes (n_peaks, n_levels, 4)
        z_scaling_factor: Scaling factor for z-axis (computed from posterior height)
        active_curve_color: Color for the contours/boxes/text
        identifier_suffix: Optional suffix for mesh names
    ...
    if identifier_suffix is None:
        curr_common_identifier_prefix_name: str = f'posterior_peaks'
    else:
        curr_common_identifier_prefix_name: str = f'posterior_peaks_{identifier_suffix}'
    ...
    curr_contours = posterior_pdata.contour(isosurfaces=z_scaling_factor*flat_peak_levels)
    try:
        contours_mesh_actor = plotter.add_mesh(curr_contours, color=active_curve_color, line_width=3, name=curr_contours_mesh_name, render=render)
    ...
    if debug_print:
        print(f'np.shape(peak_level_bboxes): {np.shape(peak_level_bboxes)}')
    ...
    for peak_idx in np.arange(n_peaks):
        for level_idx in np.arange(n_levels):
            if include_contour_bounding_box:
                ...
                out_pf_box_actors[curr_box_mesh_name] = plotter.add_mesh(out_pf_box_data[curr_box_mesh_name], color=active_curve_color,  name=curr_box_mesh_name, show_edges=True, edge_color=active_curve_color, line_width=1.5, opacity=0.75, label=curr_box_mesh_name, style='wireframe', render=render)
            ...
            if include_text_labels:
                ...
                out_pf_text_size_actors[curr_text_label_mesh_x_name] = plotter.add_mesh(x_text_mesh, name=curr_text_label_mesh_x_name, render=render)
                out_pf_text_size_actors[curr_text_label_mesh_y_name] = plotter.add_mesh(y_text_mesh, name=curr_text_label_mesh_y_name, render=render)
            # END if include_text_labels...
    ...
    if include_text_labels:    
        ...
        plotActors_labels, data_dict_labels = _perform_plot_point_labels(plotter, points, point_labels=point_labels, point_mask=point_mask,
    ...
    all_peaks_actors = CascadingDynamicPlotsList(contours=CascadingDynamicPlotsList(**out_pf_contours_actors),
                                                  boxes=CascadingDynamicPlotsList(**out_pf_box_actors),
                                                  text=CascadingDynamicPlotsList(**out_pf_text_size_actors),
                                                  peak_points=CascadingDynamicPlotsList(**out_pf_peak_points_actors),
                                                  )
...
def _plot_pyvista_single_neuron_prominence_result(ipcDataExplorer, valid_neuron_id, active_curve_color, curr_pdata, peak_locations, peak_labels, peak_levels, peak_level_bboxes, identifier_suffix: Optional[str]=None, render:bool=True, debug_print:bool=False, include_text_labels:bool=False, include_contour_bounding_box:bool=False, **kwargs):
    """ Plots a single set of contours/points/labels
        ...
        render, peak_level_bboxes, peak_labels, peak_labels, peak_labels
        
        
    curr_pdata = curr_neuron_plot_data['pdata_currActiveNeuronTuningCurve']
    all_peaks_actors, all_peaks_data = _plot_pyvista_single_neuron_prominence_result(ipcDataExplorer, valid_neuron_id=valid_neuron_id, active_curve_color=active_curve_color, curr_pdata=curr_pdata,
                                                             peak_locations=peak_locations, peak_labels=peak_labels, peak_levels=peak_levels, peak_level_bboxes=peak_level_bboxes,
                                                             identifier_suffix=None, render=render, debug_print=debug_print)
...
@function_attributes(short_name=None, tags=['peak_prominence', 'pyvista'], input_requires=[], output_provides=[], uses=['_perform_plot_point_labels', '_build_pyvista_single_neuron_prominence_result_data', '_plot_pyvista_single_neuron_prominence_result'], used_by=['render_all_neuron_peak_prominence_2d_results_on_pyvista_plotter'], creation_date='2024-05-09 05:28', related_items=[])
def _render_peak_prominence_2d_results_on_pyvista_plotter(ipcDataExplorer, active_peak_prominence_2d_results, valid_neuron_id:int=2, render=True, debug_print=True, **kwargs):
    ...
    all_peaks_data = None
    if curr_neuron_plot_data is not None:
        ...
        if len(curr_neuron_plot_data_dict) == 0:
            ...
            all_peaks_actors, all_peaks_data = _plot_pyvista_single_neuron_prominence_result(ipcDataExplorer, curr_pdata=curr_pdata, identifier_suffix=None, **_common_plot_kwargs)
            ...
            for series_identifier, a_curr_neuron_plot_data in curr_neuron_plot_data_dict.items():
                ...
                if curr_pdata is not None:
                    ...
                    an_all_peaks_actors, an_all_peaks_data = _plot_pyvista_single_neuron_prominence_result(ipcDataExplorer, curr_pdata=curr_pdata, identifier_suffix=series_identifier, **_common_plot_kwargs)
                    ...
                    if all_peaks_data is None:
                        ...
                        for k, v in an_all_peaks_data.items():
                            all_peaks_data[k] |= v
            ## END for series_identifier, a_curr_neur...
    ## END if curr_neuron_plot_data is not None...           
...
@function_attributes(short_name=None, tags=['peak_prominence', 'pyvista', 'posterior'], input_requires=[], output_provides=[], uses=['_build_pyvista_single_posterior_prominence_result_data', '_plot_pyvista_single_posterior_prominence_result'], used_by=['render_posterior_peak_prominence_2d_results_on_pyvista_plotter'], creation_date='2026-01-07 07:00', related_items=[])
def _render_posterior_peak_prominence_2d_results_on_pyvista_plotter(plotter, posterior_pdata, peak_prominence_result, epoch_idx, time_bin_idx, render=True, debug_print=True, **kwargs):
    """ Draws the 2D slice of the posterior peak prominence around its surface FOR A SINGLE (epoch_idx, time_bin_idx)
    
    Similar to _render_peak_prominence_2d_results_on_pyvista_plotter but works with posteriors instead of tuning curves.
    ...
    Args:
        ...
        time_bin_idx: Time bin index within the epoch
        render: Whether to render immediately
        debug_print: Whether to print debug info
        **kwargs: Additional arguments passed to plotting functions
    
    Returns:
        all_peaks_data, all_peaks_actors: Data and actors for the rendered peaks
    """
    result_key = (epoch_idx, time_bin_idx)
    assert result_key in peak_prominence_result.results, f"result_key {result_key} must be in the results keys, but it is not. results keys: {list(peak_prominence_result.results.keys())[:10]}..."
    
    peak_locations, prominence_array, peak_labels, peak_levels, peak_level_bboxes = _build_pyvista_single_posterior_prominence_result_data(epoch_idx, time_bin_idx, peak_prominence_result.results[result_key], promenence_plot_threshold=kwargs.get('promenence_plot_threshold', 0.2), included_level_indicies=kwargs.get('included_level_indicies', [1]), debug_print=debug_print)
    ...
    ## Convert the z components from unit-space to the z-space of the posterior:
    peak_locations[:,2] = peak_locations[:,2] * curr_scale_z
    if debug_print:
        print(f'peak_locations (scaled): {peak_locations}')
    ## Also scale the peak_levels (which are also expressed as z-positions:
    peak_levels = peak_levels * curr_scale_z
    if debug_print:
        print(f'peak_levels (scaled): {peak_levels}')

    #TODO 2026-01-07 12:08: - [ ] by adjusting and then passing `z_scaling_factor` do these things get double scaled?
    _common_plot_kwargs = dict(peak_locations=peak_locations, peak_labels=peak_labels, peak_levels=peak_levels, peak_level_bboxes=peak_level_bboxes,
                            z_scaling_factor=curr_scale_z, active_curve_color=active_curve_color,
                            include_contour_bounding_box=kwargs.get('include_contour_bounding_box', False), include_text_labels=kwargs.get('include_text_labels', False), render=render, debug_print=debug_print)
    
    ### Add pyvista contours:
    all_peaks_actors, all_peaks_data = _plot_pyvista_single_posterior_prominence_result(plotter, posterior_pdata=posterior_pdata, identifier_suffix=None, **_common_plot_kwargs)
...
@function_attributes(short_name=None, tags=['display', '3D', 'posterior', 'peaks', 'prominence'], input_requires=[], output_provides=[], uses=['_render_posterior_peak_prominence_2d_results_on_pyvista_plotter'], used_by=[], creation_date='2026-01-07 07:00', related_items=[])
def render_posterior_peak_prominence_2d_results_on_pyvista_plotter(decoded_trajectory_plotter, peak_prominence_result, promenence_plot_threshold=0.2, included_level_indicies=[1], include_contour_bounding_box=False, include_text_labels=False, debug_print=False, **kwargs):
    """
    Sets peak prominence results on a DecodedTrajectoryPyVistaPlotter and triggers rendering.
    
    This is a convenience function that sets the peak prominence result on the plotter,
    which will automatically render peaks when the time bin changes.
    
    Inputs:
        `decoded_trajectory_plotter`: a DecodedTrajectoryPyVistaPlotter instance
        `peak_prominence_result`: PosteriorPeaksPeakProminence2dResult object containing peak prominence data
        
    Provides: 
        Sets decoded_trajectory_plotter.peak_prominence_result and triggers initial render
    ...
    """
    # Set the peak prominence result on the plotter
    decoded_trajectory_plotter.set_peak_prominence_result(
        peak_prominence_result,
    ...
    return decoded_trajectory_plotter
