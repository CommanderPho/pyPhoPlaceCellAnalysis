...
class EpochRenderConfigWidget(pg.Qt.QtWidgets.QWidget):
    ...
    def __init__(self, config: Optional[EpochDisplayConfig]=None, parent=None): # 
        ...
        self.initUI()
        ## NOTE: do NOT call self.show() here. As a child widget it must not self-show (doing so briefly spawns a top-level window per widget, which produced the popup storm + slow build for many-row series). The containing `EpochRenderConfigsListWidget` is shown by the parent plot.
...
""")
            ...
            for i, a_sub_config in enumerate(a_config):
                a_sub_curr_widget = build_single_epoch_display_config_widget(a_sub_config, is_config_list=True, parent=a_sub_config_widget_container)
    ...
    def initUI(self):
        ...
        ## suppress any incidental `sigAnyConfigChanged` emitted while mass-creating widgets during initial build
...
def build_single_epoch_display_config_widget(render_config: EpochDisplayConfig, is_config_list: bool=False, parent=None) -> EpochRenderConfigWidget:
    ...
    curr_widget = EpochRenderConfigWidget(config=render_config, parent=parent) # new widget type. Pass parent so it's created as a child immediately (never a transient top-level window).
