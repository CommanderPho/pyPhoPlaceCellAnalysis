...
class PosteriorPeaksPeakProminence2dResult(ComputedResult):
    ...
    def init_from_old_PeakProminence2D_result_dict(cls, active_peak_prominence_2d_results) -> "PosteriorPeaksPeakProminence2dResult":
        ...
        a_pf_promenence_result_obj.add_computed_peak_df_columns()
    ...
    def add_computed_peak_df_columns(self):
        """ 
        
        adds ['summit_slice_percent_area', ] columns
        """
        ## compute the total possible area (enviornment area)
        environment_possible_size = (np.ptp(self.xx), np.ptp(self.yy))
        ...
        # a_df_list = [self.flat_peaks_df, self.filtered_flat_peaks_df]

        if 'summit_slice_area' in self.flat_peaks_df.columns:
            self.flat_peaks_df['summit_slice_percent_area'] = self.flat_peaks_df['summit_slice_area'] / environment_possible_area
            
        if 'summit_slice_area' in self.filtered_flat_peaks_df.columns:
            self.filtered_flat_peaks_df['summit_slice_percent_area'] = self.filtered_flat_peaks_df['summit_slice_area'] / environment_possible_area
                

        # for a_df in a_df_list:
        #     if 'summit_slice_area' in a_df.columns:
        #         a_df['summit_slice_percent_area'] = a_df['summit_slice_area'] / environment_possible_area
...
class PeakPromenence:
    ...
    @classmethod
    def isContClosed(cls, contour):
        x=contour.vertices[:, 0]
        y=contour.vertices[:, 1]
        return cls.isClosed(x,y)


    @classmethod
    def polygonArea(cls, x, y):
        if not cls.isClosed(x,y):
    ...
    def contourArea(cls, contour):
        ...
        if len(segs)>1:
            ...
            for pp in segs:
                xii=pp[:,0]
                yii=pp[:,1]
                areaii=cls.polygonArea(xii,yii)
                areas.append(areaii)
            areas.sort()
            ...
            x=contour.vertices[:, 0]
            y=contour.vertices[:, 1]
            result=cls.polygonArea(x,y)

        return result


    @classmethod
    def polygonGeoArea(cls, lons, lats, method='basemap', projection='cea', bmap=None, verbose=True):
        ...
        result=cls.polygonArea(xs,ys)

        return result


    @classmethod
    def contourGeoArea(cls, contour, bmap=None):
        ...
        if len(segs)>1:
            ...
            for pp in segs:
                xii=pp[:,0]
                yii=pp[:,1]
                areaii=cls.polygonGeoArea(xii,yii,bmap=bmap)
                areas.append(areaii)
            areas.sort()
            result=areas[-1]-np.sum(areas[:-1])
        else:
            x=contour.vertices[:, 0]
            y=contour.vertices[:, 1]
            result=cls.polygonGeoArea(x,y,bmap=bmap)

        return result


    @classmethod
    def getProminence(cls, var, step, ybin_centers=None, xbin_centers=None, min_considered_promenence=None, include_edge=True, min_area=None, max_area=None, area_func=None, centroid_num_to_center=5, allow_hole=True, max_hole_area=None, verbose=False):
        ...
        if area_func==cls.contourGeoArea:
            from mpl_toolkits.basemap import Basemap
        ...
        for ii,levii in enumerate(levels[::-1]):
            ...
            for jj, contjj in enumerate(csii.get_paths()):
                ...
                if not include_edge and not cls.isContClosed(contjj):
                ...
                if len(segs)>1:
                    ...
                    if not allow_hole:
                        ...
                        if max_hole_area is not None:
                            areas=[]
                            if area_func==cls.contourGeoArea:
                                areas=[cls.polygonGeoArea(segkk[:,0],segkk[:,1],\
                                    bmap=bmap)/1e6 for segkk in segs]
                            else:
                                areas=[cls.polygonArea(segkk[:,0],segkk[:,1])\
                                        for segkk in segs]
                            areas.sort()
    ...
    @classmethod
    def compute_prominence_contours(cls, xbin_centers: NDArray, ybin_centers: NDArray, slab: NDArray, step: float=0.1, min_area: Optional[float]=None, min_considered_promenence: float=0.2, include_edge: bool=True, verbose: bool=False, **kwargs):
        ...
        """
        peaks_dict, id_map, prominence_map, parent_map = cls.getProminence(slab, step, ybin_centers=ybin_centers, xbin_centers=xbin_centers, min_area=min_area, min_considered_promenence=min_considered_promenence, include_edge=include_edge, verbose=verbose, **kwargs)
    ...
    def _compute_single_posterior_slab(cls, epoch_idx: int, t_idx: int, slab: NDArray, xbin_centers: NDArray, ybin_centers: NDArray, step: float, min_considered_promenence: float, peak_height_multiplier_probe_levels: Tuple, debug_print: bool = False, should_return_raw_matplotlib_Path_contours: bool=False):
        ...
        _, _, slab, peaks_dict, id_map, prominence_map, parent_map = cls.compute_prominence_contours(
        ...
        for peak_idx, (peak_id, a_peak_dict) in enumerate(peaks_dict.items()):
            ...
            a_peak_dict['level_slices'] = {
                probe_lvl: {
                    ...
                    'area': cls.contourArea(contour) if (contour is not None) else None,  # Add this
                }
    ...
    def _build_filtered_summits_analysis_results(cls, xbin, ybin, xbin_labels, ybin_labels, flat_peaks_df: pd.DataFrame, active_eloy_analysis=None, slice_level_multiplier=0.5, minimum_included_peak_height=0.5, debug_print=False):
        ...
        if minimum_included_peak_height is not None:
            ...
            filtered_summits_analysis_df = flat_peaks_df.copy() # filter for peaks greater than 1.0Hz
    ...
    def compute_2d_dt_posterior_peak_promenences(cls, a_p_x_given_n: NDArray[ND.Shape["N_XBINS, N_YBINS, N_TBINS"], Any], alpha: Union[float, List[float]] = 0.9, memory_warn_bytes: Optional[int] = int(60 * 1024**3), memory_strict: bool = True):
        ...
        memory_warn_bytes: if set, warn (or raise when memory_strict) when estimated output mask bytes exceed this.
        memory_strict: when True and memory_warn_bytes is set, raise MemoryError instead of warning.
        """
        # ==================================================================================================================================================================================================================================================================================== #
        # BEGIN FUNCTION BODY                                                                                                                                                                                                                                                                  #
        # ==================================================================================================================================================================================================================================================================================== #
        ...
        n_x_bins, n_y_bins, n_t_bins = a_p_x_given_n.shape[0], a_p_x_given_n.shape[1], np.shape(a_p_x_given_n)[-1]
        n_alpha = len(alpha)
        estimated_mask_bytes = n_x_bins * n_y_bins * n_t_bins * n_alpha * np.dtype(bool).itemsize
        if memory_warn_bytes is not None and estimated_mask_bytes > memory_warn_bytes:
            msg = f"compute_2d_dt_posterior_peak_promenences estimated mask memory {estimated_mask_bytes / (1024**2):.1f} MiB exceeds threshold {memory_warn_bytes / (1024**2):.1f} MiB (shape {n_x_bins}x{n_y_bins}x{n_t_bins}, {n_alpha} alpha)."
            if memory_strict:
    ...
    def track_peaks_across_trials(peak_prominence_df: pd.DataFrame, trial_order_col: str = 'rel_trial_idx',
        ...
        """
        Assign stable aclu_field_peak_id per (decoder_name, aclu), reset within each aclu group, and build transition rows.

        ``aclu_field_peak_id`` is unique within each (decoder_name, aclu) group (starts at 0 per aclu).
        ``global_field_peak_id`` is globally unique across all groups/aclus.

        Returns
        -------
        tracked_peaks_df : original rows + aclu_field_peak_id + global_field_peak_id
        ...
        for group_key, aclu_df in peak_prominence_df.groupby(group_cols, sort=False):
            ...
            # trial -> {original_row_index: (aclu_field_peak_id, global_field_peak_id)}
            ...
            for trial_i, rel_trial in enumerate(sorted_trials):
                ...
                if trial_i == 0:
                    ...
                    for _, row in trial_df.iterrows():
                        track_map[int(row['index'])] = (next_aclu_field_peak_id, next_global_field_peak_id)
                        next_aclu_field_peak_id += 1
                ...
                for prev_idx, next_idx in matched_pairs:
                    aclu_field_peak_id, global_field_peak_id = prev_track_map[prev_idx]
                    track_map[next_idx] = (aclu_field_peak_id, global_field_peak_id)
                    ...
                    transition_rows.append({
                        **dict(zip(group_cols, group_key)),
                        ...
                        'aclu_field_peak_id': aclu_field_peak_id,
                ...
                for next_idx in unmatched_next:
                    aclu_field_peak_id = next_aclu_field_peak_id
            ...
            for rel_trial, trial_df in trial_groups.items():
                ...
                for _, row in trial_df.iterrows():
                    ...
                    aclu_field_peak_id, global_field_peak_id = track_map[int(row['index'])]
                    out_row['aclu_field_peak_id'] = aclu_field_peak_id
...
class PeakPromenenceDisplay:
    ...
    def plot_Prominence(cls, xx, yy, slab, peaks, idmap, promap, parentmap, n_contour_levels=None, debug_print=False):
        ...
        Usage:
            ...
            zmax = slab.max()
            peaks, idmap, promap, parentmap = PeakPromenence.getProminence(slab, step, ybin_centers=yy, xbin_centers=xx, min_area=None, include_edge=True, verbose=False)
            figure, (ax1, ax2, ax3, ax4) = PeakPromenenceDisplay.plot_Prominence(xx, yy, slab, peaks, idmap, promap, parentmap, debug_print=False)
