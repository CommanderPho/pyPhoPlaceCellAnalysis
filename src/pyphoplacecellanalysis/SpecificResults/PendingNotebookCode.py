...
class HeuristicReplayScoring:
    ...
    from pyphoplacecellanalysis.SpecificResults.PendingNotebookCode import HeuristicReplayScoring, HeuristicScoresTuple
    ...
    def compute_pho_heuristic_replay_scores(cls, a_result: DecodedFilterEpochsResult, an_epoch_idx: int = 1, debug_print=False, enable_debug_plot=False, debug_plot_axs=None, common_plot_kwargs=None, debug_plot_name=None, **kwargs) -> HeuristicScoresTuple:
        ...
        if enable_debug_plot:
            ...
            fig, debug_plot_axs = debug_plot_helper_add_position_and_derivatives(time_window_centers, position, velocity, acceleration,
    ...
    def compute_all_heuristic_scores(cls, track_templates: TrackTemplates, a_decoded_filter_epochs_decoder_result_dict: Dict[str, DecodedFilterEpochsResult]):
        ...
        def sweep_score(positions):
            ...
            return len(unique_positions) / len(np.unique(positions))
        ...
        _positions_fns = [directionality_ratio, sweep_score] # , transition_entropy 
        ...
        for a_name, a_result in a_decoded_filter_epochs_decoder_result_dict.items():
            ...
            _out_new_scores[a_name] =  pd.DataFrame([asdict(cls.compute_pho_heuristic_replay_scores(a_result=a_result, an_epoch_idx=an_epoch_idx, enable_debug_plot=False, debug_plot_axs=None, debug_plot_name=a_name), filter=lambda a, v: a.name not in ['position_derivatives_df']) for an_epoch_idx in np.arange(a_result.num_filter_epochs)])
    ...
    def _DEP_run_all_compute_pho_heuristic_replay_scores(cls, filter_epochs: pd.DataFrame, a_decoded_filter_epochs_decoder_result_dict, t_start, t_delta, t_end, labels_column_name='lap_id'):
        ...
        for i, row in enumerate(filter_epochs.itertuples()): # np.arange(num_filter_epochs)
            ...
            _out_true_decoder_new_scores[i] = cls.compute_pho_heuristic_replay_scores(a_result=a_result, an_epoch_idx=i, debug_plot_axs=None, debug_plot_name=None)
