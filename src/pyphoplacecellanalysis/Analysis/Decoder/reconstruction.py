...
class DecodedFilterEpochsResult(HDF_SerializationMixin, AttrsBasedClassHelperMixin):
    ...
    @function_attributes(short_name=None, tags=['mask', 'unit-spike-counts', 'pure'], input_requires=[], output_provides=[], uses=['self.mask_computed_DecodedFilterEpochsResult_by_time_bin_inclusion_masks', 'spikes_df.spikes.compute_unit_time_binned_spike_counts_and_mask'], used_by=[], creation_date='2025-03-04 01:32', related_items=[])
...
class BasePositionDecoder(HDFMixin, AttrsBasedClassHelperMixin, ContinuousPeakLocationRepresentingMixin, PeakLocationRepresentingMixin, NeuronUnitSlicableObjectProtocol, BinnedPositionsMixin):
    ...
    Usage:
        ...
        from pyphoplacecellanalysis.Analysis.reliability import ReliabilityDecoderModifierMode
...
# ==================================================================================================================== #
@custom_define(slots=False, eq=False)
class BayesianPlacemapPositionDecoder(SerializedAttributesAllowBlockSpecifyingClass, CellIndividualReliabilityComputingMixin, BasePositionDecoder): # needs `HDFMixin, `
    ...
    reliability_estimation_mode: ReliabilityEstimationMode = non_serialized_field(default=ReliabilityEstimationMode.POSITION_DEPENDENT)
    ...
    aclu_tbin_confusion_lap_df: pd.DataFrame = serialized_field(default=None, is_computable=True, metadata={'shape': ('n_aclus_x_known_tbins',)})
    ...
    def from_dict(cls, val_dict):
        new_obj = cls(time_bin_size=val_dict.get('time_bin_size', 0.25), pf=val_dict.get('pf', None), spikes_df=val_dict.get('spikes_df', None), setup_on_init=val_dict.get('setup_on_init', True), post_load_on_init=val_dict.get('post_load_on_init', False), debug_print=val_dict.get('debug_print', False))
        return new_obj
