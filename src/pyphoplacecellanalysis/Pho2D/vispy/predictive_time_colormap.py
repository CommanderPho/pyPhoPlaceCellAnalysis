...
PredictiveTimeColormapName = Literal["cyan_magenta", "plasma", "cool", "rainbow_hue"]

PREDICTIVE_TIME_COLORMAP_NAMES: tuple[PredictiveTimeColormapName, ...] = ("cyan_magenta", "plasma", "cool", "rainbow_hue")

# Matplotlib colormap name per predictive-time key (None = custom path: cyan_magenta stops or rainbow_hue HSV).
...
# HSV saturation/value for rainbow_hue (legacy full-spectrum time bins).
_RAINBOW_HUE_S = 0.8
_RAINBOW_HUE_V = 0.9
...
def _predictive_time_rgb_for_key(key: PredictiveTimeColormapName, u: float) -> tuple[float, float, float]:
    ...
    if key == "rainbow_hue":
        rgb = colorsys.hsv_to_rgb(uu, _RAINBOW_HUE_S, _RAINBOW_HUE_V)
...
def predictive_time_rgb(u: float, colormap: PredictiveTimeColormapName | str | None = None) -> tuple[float, float, float]:
    """Sample the sequential time colormap at u in [0, 1] (clamped in the resolver). Returns RGB in [0, 1]. Uses ``colormap`` or the module default."""
