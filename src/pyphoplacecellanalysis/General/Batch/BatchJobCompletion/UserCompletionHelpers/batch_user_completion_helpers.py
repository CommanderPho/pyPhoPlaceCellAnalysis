...
def perform_sweep_decoding_time_bin_sizes_marginals_dfs_completion_function(self, global_data_root_parent_path, curr_session_context, curr_session_basedir, curr_active_pipeline, across_session_results_extended_dict: dict,
    ...
    if custom_all_param_sweep_options is None:
        if desired_shared_decoding_time_bin_sizes is None:
            ...
            desired_shared_decoding_time_bin_sizes = np.array([0.025, 0.075])
    ...
    assert 'maze_id' in laps_df.columns, f"laps_df is still missing the 'maze_id' column after calling `laps_obj.update_maze_id_if_needed(...)`. laps_df.columns: {str(list(laps_df.columns))}"
        ...
        try:
            ...
            if out_custom_formats_dict is not None:
                custom_merge_layout_dict = [
                    # ['greyscale'], #TODO 2025-09-30 06:37: - [ ] Explicitly exclude the non-globally normalized column upon Kamran's request. Now will be a single-column per event. 
